...
class MipNerfModel(nn.Module):
  ...
  def __call__(
    ...
    for i_level in range(self.num_levels):
      ...
      ## blending weight regeneration
      weights_regen, _, _, _, _ = mip.compute_alpha_weights(
        density, t_vals, mu_depth, weights_aux=True)

      pi = weights / jnp.sum(weights, axis=1, keepdims=True)
      ## pi regeneration
      pi_regen = weights_regen / jnp.sum(weights_regen, axis=1, keepdims=True)
...
class MLP(nn.Module):
  ...
  gamma_activation: Callable[Ellipsis, Any] = nn.softplus
  num_gamma_channels: int = 3
  ...
  ## ---------------------------- ##
    ...
    gamma = self.gamma_activation(dense_layer(self.num_gamma_channels)(x))
    mu_depth = jnp.linalg.norm(dense_layer(self.num_mudepth_channels)(x), axis=-1)

    return (rgb, density, normals, gamma, mu_depth)
